// "use client";
// import Image from "next/image";
// import { HiArrowNarrowRight } from "react-icons/hi";
// import { useState } from "react";
// import "slick-carousel/slick/slick.css";
// import "slick-carousel/slick/slick-theme.css";
// import dynamic from "next/dynamic";
// import Link from "next/link";
// const Slider = dynamic(() => import("react-slick"), {
//   ssr: false,
//   loading: () => <div></div>
// });

// const ProductSlider = ({ products }) => {
//   const productsNumber = [1, 2, 3, 4, 5, 6];
//   const [currentSlide, setCurrentSlide] = useState(0);

//   const settings = {
//     infinite: true,
//     slidesToScroll: 1,
//     autoplay: true,
//     autoplaySpeed: 3500,
//     pauseOnHover: false,
//     centerPadding: "20px",
//     draggable: false,
//     swipe: false,
//     touchMove: false,
//     beforeChange: (current, next) =>
//       setCurrentSlide(next % productsNumber.length),
//     responsive: [
//       {
//         breakpoint: 410,
//         settings: {
//           slidesToShow: 1,
//         },
//       },
//       {
//         breakpoint: 650,
//         settings: {
//           slidesToShow: 2,
//         },
//       },
//       {
//         breakpoint: 880,
//         settings: {
//           slidesToShow: 3,
//         },
//       },
//       {
//         breakpoint: 1024,
//         settings: {
//           slidesToShow: 4,
//         },
//       },
//       {
//         breakpoint: 1240,
//         settings: {
//           slidesToShow: 5,
//         },
//       },
//     ],
//   };

//   return (
//     <Slider {...settings} className="mt-4">
//       {products.map((p, index) => {
//         const isActive = currentSlide === index;
//         return (
//           <div key={p._id} className="relative col-span-1 w-44 h-44">
//             <div
//               className={`w-full h-full bg-white rounded-lg custom-shadow origin-bottom transition-all duration-500 ${
//                 isActive && "lg:animate-plant"
//               }`}
//             ></div>
//             <div
//               className={`absolute bottom-0 transition-all duration-500 -left-[2.6rem] w-[230px] h-[230px] ${
//                 isActive
//                   ? "lg:-left-[6.5rem] lg:w-[450px] lg:h-[450px]"
//                   : "-left-[2.6rem] w-[230px] h-[230px]"
//               }`}
//             >
//               <Image
//                 src={p.images}
//                 alt={p.name}
//                 fill
//                 className="object-contain"
//               />
//             </div>
//             <p
//               className={`absolute z-30 transition-all duration-500 ${
//                 isActive
//                   ? "bottom-16 right-[15%] lg:right-[25%] text-black"
//                   : "bottom-5 right-5 text-secondary-500"
//               } text-sm font-semibold`}
//             >
//               {p.name}
//             </p>
//             {isActive && (
//               <Link href={`/products/${p._id}`}>
//                 <p className="transition-all duration-500 absolute bottom-10 right-[13%] lg:right-[25%] text-primary-500 flex items-center text-xs">
//                   View the product <HiArrowNarrowRight />
//                 </p>
//               </Link>
//             )}
//           </div>
//         );
//       })}
//     </Slider>
//   );
// };

// export default ProductSlider;

"use client";
import Image from "next/image";
import { HiArrowNarrowRight } from "react-icons/hi";
import { useState, useEffect, useRef } from "react";
import Link from "next/link";
import { Swiper, SwiperSlide } from "swiper/react";
import { Autoplay } from "swiper/modules";
import "swiper/css";
import "swiper/css/autoplay";

const ProductSlider = ({ products }) => {
  const [isMounted, setIsMounted] = useState(false);
  const swiperRef = useRef(null);
  const [activeIndex, setActiveIndex] = useState(0);

  useEffect(() => {
    setIsMounted(true);
  }, []);

  if (!isMounted) {
    return (
      <div className="mt-4 flex gap-4">
        {[1, 2, 3, 4, 5].map((item) => (
          <div key={item} className="w-44 h-44 bg-gray-200 rounded-lg animate-pulse"></div>
        ))}
      </div>
    );
  }

  return (
    <div className="mt-4">
      <Swiper
        ref={swiperRef}
        modules={[Autoplay]}
        spaceBetween={20}
        slidesPerView={5}
        autoplay={{
          delay: 3500,
          disableOnInteraction: false,
        }}
        speed={500}
        onSlideChange={(swiper) => setActiveIndex(swiper.realIndex)}
        breakpoints={{
          1024: {
            slidesPerView: 4,
          },
          880: {
            slidesPerView: 3,
          },
          650: {
            slidesPerView: 2,
          },
          410: {
            slidesPerView: 1,
          },
        }}
        className="product-swiper"
      >
        {products.map((p, index) => {
          const isActive = activeIndex === index;
          return (
            <SwiperSlide key={p._id}>
              <div className="relative col-span-1 w-44 h-44 mx-auto">
                <div
                  className={`w-full h-full bg-white rounded-lg custom-shadow origin-bottom transition-all duration-500 ${
                    isActive && "lg:animate-plant"
                  }`}
                ></div>
                <div
                  className={`absolute bottom-0 transition-all duration-500 -left-[2.6rem] w-[230px] h-[230px] ${
                    isActive
                      ? "lg:-left-[6.5rem] lg:w-[450px] lg:h-[450px]"
                      : "-left-[2.6rem] w-[230px] h-[230px]"
                  }`}
                >
                  <Image
                    src={p.images}
                    alt={p.name}
                    fill
                    className="object-contain"
                    sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                  />
                </div>
                <p
                  className={`absolute z-30 transition-all duration-500 ${
                    isActive
                      ? "bottom-16 right-[15%] lg:right-[25%] text-black"
                      : "bottom-5 right-5 text-secondary-500"
                  } text-sm font-semibold`}
                >
                  {p.name}
                </p>
                {isActive && (
                  <Link 
                    href={`/products/${p._id}`}
                    className="transition-all duration-500 absolute bottom-10 right-[13%] lg:right-[25%] text-primary-500 flex items-center text-xs"
                  >
                    View the product <HiArrowNarrowRight />
                  </Link>
                )}
              </div>
            </SwiperSlide>
          );
        })}
      </Swiper>
    </div>
  );
};

export default ProductSlider;
